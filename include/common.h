...
const static float throw_probability[] = {
...
#define GET_PIECE_MASK(var_piece_mask, piece_index)                            \
  uint32_t(var_piece_mask) = MASK_PIECE((piece_index))

#define PIECE_FIELD_SET(piece_index, pos)                                      \
  (((uint32_t)pos) << (4 * (piece_index)))
