...
static float points_base[] = {100, -.5, 1, 2,  3,  4,  5,  6,
...
const static minimax_config_t config = {
    ...
    .eval_config = {.points_base = points_base,
                    .adder_kill_possible = 10,
                    .adder_attack_possible = -1.5,
                    .adder_kill_happens = 100},
    .visualize_config = {.enable = false, .graph_path = "Graph.gv"}};

state_t *set_up_state(void) {
  const short player_current_default = 0, player_other_default = 1;

  return state_init(config.score_player_0, config.score_player_1,
                    config.pieces_player_0, config.pieces_player_1,
...
void test0() {
  ...
  const state_t *state_new =
      simulate(state_current, piece_index, dice, &config);
...
void test3() {
  ...
  cleanup_children(state_current);
  cleanup_children(state_expected);
  cleanup_children(state_new);
}
